...
def ioU_jaccard_distance(y_true, y_pred, smooth=100):
    ...
    sum_ = kb.sum(kb.abs(y_true) + kb.abs(y_pred), axis=-1)
    print("intersection")
...
def dummy_indicator(y_predicted, thres=0.0):
    ...
    y_predicted = kb.tf.where(kb.greater(y_predicted, thres),kb.ones_like(y_predicted), kb.zeros_like(y_predicted))
...
def vary_boolean_jaccard(y_true, y_predicted):
    ...
    intersection = kb.sum(kb.abs(y_true - y_predicted), axis=-1)
    union = kb.sum(kb.ones_like(y_predicted), axis=-1) # number of possible matches (tensors y_predicted, y_true have the same number
    # of values)
    j_index = kb.tf.divide(intersection, union)
